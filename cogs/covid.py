...
class Covid(commands.Cog):
    ...
    @commands.command(brief="Covid-19 stats!",help="Covid stats.",aliases=['cv'])
    async def covid(self, ctx, *, countryName = None):
        try:
            if countryName is None:
                embed=discord.Embed(title=f"This command is used like this: ```{ctx.prefix}covid [country]```", colour=0x2f3136, timestamp=ctx.message.created_at)
            ...
            embed3 = discord.Embed(title="Invalid Country Name Or API Error.", colour=0x2f3136, timestamp=ctx.message.created_at)
